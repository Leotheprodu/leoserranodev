...
highlights:
  ...
  - 'Login via WhatsApp bot + Google OAuth + JWT with auto-refresh'
...
No passwords. New users send a WhatsApp message to the bot number
(`+50663017707`) with a special format
(`registro-adorador:<token>`). The bot confirms the email or phone
and the user is in. Forgot password? Same flow.

Three dedicated docs cover the migration:
`WHATSAPP_AUTH_MIGRATION.md`, `WHATSAPP_AUTH_FRONTEND.md`,
`WHATSAPP_ERROR_FIX.md`. Plus Google OAuth as a second path for
musicians who already have an account.
